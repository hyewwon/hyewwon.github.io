...
await copyFile(source,path.join(out,'source/frog-experience-generated-v2.png'));
...
// Freeze the whole idle body; only the eye band changes during a blink.
for (const i of [13,14]) {
 const fixed=Buffer.from(frames[12]);
 // Both pupils lie inside this facial band, below the unmodified head outline.
 for(let y=82;y<119;y++)for(let x=170;x<279;x++){
  const p=(y*384+x)*4;
  frames[i].copy(fixed,p,p,p+4);
 }
 frames[i]=fixed;
}
const atlas=Buffer.alloc(1536*1536*4);
for(let i=0;i<16;i++){await sharp(frames[i],{raw:{width:384,height:384,channels:4}}).png().toFile(path.join(out,'frames-v2',`frame-experience-v2-${String(i+1).padStart(2,'0')}.png`));for(let y=0;y<384;y++)frames[i].copy(atlas,((Math.floor(i/4)*384+y)*1536+i%4*384)*4,y*384*4,(y+1)*384*4);}
const output=path.join(out,'sprites/frog-experience-walk-sheet-v2.png');await sharp(atlas,{raw:{width:1536,height:1536,channels:4}}).png().toFile(output);
...
for(const i of [13,14])for(let y=0;y<384;y++)for(let x=0;x<384;x++){
 if(y>=82&&y<119&&x>=170&&x<279)continue;
 const p=(y*384+x)*4;
 assert(frames[i].subarray(p,p+4).equals(frames[12].subarray(p,p+4)),'Blink body must remain identical');
}
await writeFile(path.join(out,'quality-v2.json'),JSON.stringify({sourcePalette,targetPalette,scale,firstFrameExact:true,restAndBlinkReturnExact:true},null,2)+'\n');
console.log('PASS 16 RGBA frames; Skills handoff exact; blink returns to identical standing pose; palette matched', {sourcePalette,targetPalette,scale});
