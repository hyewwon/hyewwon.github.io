(() => {
  ...
  const projects = [
    ...
    // Keep review scores, colleagues' feedback, and internal URLs out of public content.
    {
      ...
      contributions: [
        '이미지 생성 API: 선임 개발자들과 이미지 생성 관련 API 개발 참여 및 생성 작업 연동 지원',
        '비동기 처리: 이미지 생성 서비스의 비동기 처리 개발 참여 및 선임 개발자의 생성 작업 팔로우업',
        'CMS 화면 연동: Fetch 기반 데이터 조회·폼 전송·응답 처리 구현. 화면의 조회·렌더링 흐름과 팀원 요구사항을 고려한 응답 구조 정리 및 운영 화면 개선',
        'QA·배포 지원: QA 피드백 대응 및 배포 과정 팔로우업'
      ],
    ...
    // Evidence: works' weekly project log, May–September 2026. Do not publish internal budget amounts.
    {
      ...
      contributions: [
        '인프라 운영: 인스턴스 시작·중지·재실행 및 스냅샷 기반 생성 구현. 삭제 승인 시 인스턴스·DNS·키 페어 정리, PostgreSQL 설치 스냅샷 기반 구성 및 DB 신청·삭제 API 구현',
        '결재·권한: 관리자 승인·반려, 자원 삭제 결재 및 Station 최초 릴리즈·수정·미노출의 2단계 승인 구현. 카테고리·접근 권한 그룹 관리와 화면 연동',
        'Station 운영: 게시 여부·순서 및 히어로 배너 관리, S3 이미지 업로드·교체·정리 구현. 후속 버전 생성·삭제·릴리즈 노트 수정 및 Developers에서 Station 반응 집계 조회 연동',
        '서비스 간 연동: Developers 승인 이후 Celery를 통한 Station API 호출 및 버전·릴리즈 노트 비동기 동기화. 최초 릴리즈 버전 삭제 제한 및 Bearer Token 인증 적용',
        '인증·배포: 그룹웨어 SSO 로그인·토큰 발급·삭제 및 삭제 결재·이력 처리 연동. 스테이징·Developers 프로덕션 환경 설정, 이미지 접근·반응 API 테스트 추가'
      ],
